...
static int bflb_serial_setbrg(struct udevice *dev, int baudrate)
{
	...
	if (IS_ERR_VALUE(ret)) {
		debug("bflb UART failed to get clock\n");
		ret = dev_read_u32(dev, "clock-frequency", &clock);
		if (IS_ERR_VALUE(ret)) {
			debug("bflb UART clock not defined\n");
			return 0;
		}
	} else {
		clock = clk_get_rate(&clk);
		if (IS_ERR_VALUE(clock)) {
			debug("bflb UART clock get rate failed\n");
			return 0;
		}
	}
	plat->clock = clock;
	_bflb_serial_setbrg(plat->regs, plat->clock, baudrate);
#endif
...
static inline void _debug_uart_init(void)
{
	...
	writel(0xff32ffff, 0x20000154);
	writel(0x0000ffff, 0x20000158);
	writel(0x0f000003, 0x20000120);
	writel(0x40400717, 0x20000904);
	writel(0x40400717, 0x20000908);
