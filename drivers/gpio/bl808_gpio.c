				       ...
				       int value)
{
	...
	val |= GPIO_CFG_IE;
	val |= GPIO_CFG_OE;
	u32p_replace_bits(&val, GPIO_MODE_SET_CLR, GPIO_CFG_MODE);
	writel(val, priv->glb + GLB_GPIO_CFG(gpio));

	return bl808_gpio_set_value(dev, gpio, value);
}
...
static int bl808_gpio_probe(struct udevice *dev)
{
	...
	priv->glb = dev_read_addr_ptr(dev_get_parent(dev_get_parent(dev)));
	if (!priv->glb)
		return -EINVAL;
